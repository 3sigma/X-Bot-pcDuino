...
def CalculVitesse():
    global ticksCodeurDroit, ticksCodeurGauche, indiceTicksCodeurDroit, indiceTicksCodeurGauche, started, \
        ...
        tdebut, ad, P_x_Droit, I_x_Droit, D_x_Droit, P_x_Gauche, I_x_Gauche, D_x_Gauche, bd, Td, Tt, yprecDroit, yprecGauche, interruptKO, \
        codeurDroitDeltaPos, codeurGaucheDeltaPos, commandeDroit, commandeGauche, vxmes, xidotmes, omegarefDroit, omegarefGauche, \
    ...
    if not interruptKO:
        if started and ((ticksCodeurDroitTravail == 0) and (ticksCodeurGaucheTravail == 0)) and (((omegarefDroit != 0) and (ticksCodeurDroitTravail == 0) and abs(commandeDroit) > 2) or ((omegarefGauche != 0) and (ticksCodeurGaucheTravail == 0) and abs(commandeGauche) > 2)):
    ...
    # Terme proportionnel
    P_x_Droit = Kp * (omegarefDroit - omegaDroit)
    P_x_Gauche = Kp * (omegarefGauche - omegaGauche)
    ...
    I_x_Droit = I_x_Droit + Ki * dt * (omegarefDroit - omegaDroit) + br * (commandeDroit - commande_avant_sat_Droit)
    I_x_Gauche = I_x_Gauche + Ki * dt * (omegarefGauche - omegaGauche) + br * (commandeGauche - commande_avant_sat_Gauche)
...
class WSHandler(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, message):
        global omegaref, omegarefDroit, omegarefGauche

        jsonMessage = json.loads(message)
        
        if jsonMessage.get('omegaref') != None:
            omegaref = float(jsonMessage.get('omegaref'))
            #print ("omegaref: %.2f" % omegaref)
        ...
        omegarefDroit = moteurDroit * omegaref
        omegarefGauche = moteurGauche * omegaref
        

    def on_close(self):
        global socketOK, omegarefDroit, omegarefGauche
    ...
    def sendToSocket(self):
        global started, codeurDroitDeltaPos, codeurGaucheDeltaPos, socketOK, commandeDroit, commandeGauche, omegarefDroit, omegarefGauche, vxmes, xidotmes, interruptKO
        ...
        aEnvoyer = json.dumps({'Temps':("%.2f" % tcourant), 'ConsigneOmegaDroit':("%.2f" % omegarefDroit), 'ConsigneOmegaGauche':("%.2f" % omegarefGauche), 'Vitesse longitudinale':("%.2f" % vxmes), 'Vitesse de rotation':("%.2f" % (180 * xidotmes/3.141592)), 'omegaDroit':("%.2f" % omegaDroit), 'omegaGauche':("%.2f" % omegaGauche), 'commandeDroit':("%.2f" % commandeDroit), 'commandeGauche':("%.2f" % commandeGauche), 'Raw':("%.2f" % tcourant) + "," + ("%.2f" % omegarefDroit) + "," + ("%.2f" % omegarefGauche) + "," + ("%.2f" % vxmes) + "," + ("%.2f" % (180 * xidotmes/3.141592)) + "," + ("%.2f" % omegaDroit) + "," + ("%.2f" % omegaGauche) + "," + ("%.2f" % commandeDroit) + "," + ("%.2f" % commandeGauche)})
        if socketOK:
            try:
...
def signal_handler(signal, frame):
    global omegarefDroit, omegarefGauche
